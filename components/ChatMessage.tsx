...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  return (
    <div
      className={`flex w-full mb-4 md:mb-8 animate-in fade-in slide-in-from-bottom-2 duration-300 ${isAssistant ? "justify-start" : "justify-end"}`}
    >
      <div
        className={`flex max-w-[95%] md:max-w-[80%] ${isAssistant ? "flex-row" : "flex-row-reverse"}`}
      >
        <div
        ...
        >
          <div
            className={`markdown-content ${isAssistant ? "prose prose-slate dark:prose-invert prose-p:leading-relaxed prose-li:marker:text-slate-400 dark:prose-li:marker:text-slate-500" : "prose prose-invert prose-p:leading-relaxed prose-p:text-white/90 prose-strong:text-white prose-a:text-indigo-200"} max-w-none`}
          >
            <ReactMarkdown
